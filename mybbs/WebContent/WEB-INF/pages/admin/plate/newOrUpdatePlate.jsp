...
<body style="background: #edf6fa;">
	<h2 style="color: gray">
		&nbsp;&nbsp;&nbsp;<a href="index.jsp">主页</a> > <a href="<%=basePath%>getPlateListServlet?nowPages=1">版块管理</a> > <a>${str}版块</a>
	</h2>
	<c:if test="${bj==1 }">
		<script type="text/javascript">
			alert("修改成功");
	...
	<c:choose>
		<c:when test="${plate.id>0 }">
			<form action="<%=basePath%>updatePlateServlet" method="post">
		</c:when>
		<c:otherwise>
			<form action="<%=basePath%>newPlateServlet" method="post">
		</c:otherwise>
	</c:choose>
	<ul>
		<li>版块名称</li>
	...
	<ul>
		...
		<li><input type="text" name="info" value="${plate.info }">
			<input type="hidden" value="${plate.postNum+0}" name="postNum">
			<input type="hidden" value="${plate.id+0}" name="id"></li>
	</ul>
