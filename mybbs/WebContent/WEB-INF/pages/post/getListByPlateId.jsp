							...
							<div class="contentList">
								...
								<div align="right">
									<a class="span6 offset2 bm30"
										href="<%=basePath%>getPostListByPlateIdServlet?plateId=${plateId}&nowPages=1">首页</a>
									<c:choose>
										<c:when test="${nowPages!=1 }">
											<a class="span6 offset2 bm30"
												href="<%=basePath%>getPostListByPlateIdServlet?plateId=${plateId}&nowPages=${nowPages-1}">上一页</a>
										</c:when>
										<c:otherwise>
									...
									<c:choose>
										<c:when test="${nowPages!=commonPages.totalpages }">
											<a class="span6 offset2 bm30"
												href="<%=basePath%>getPostListByPlateIdServlet?plateId=${plateId}&nowPages=${nowPages+1}">下一页</a>
										</c:when>
										<c:otherwise>
											<a class="span6 offset2 bm30" href="javascript:return false;"
												onclick="return false;" style="cursor: default;"><i
									...
									<a class="span6 offset2 bm30"
										href="<%=basePath%>getPostListByPlateIdServlet?plateId=${plateId}&nowPages=${commonPages.totalpages}">尾页</a>
								</div>
							</div>
							<!--文章列表结束-->
